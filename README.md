...
## 🕹️ Функционал кнопок управления

### 1. Кнопка "Начать работу" 
**Назначение**: Основная кнопка активации системы  
**Функционал**:
- Инициализирует WebSocket-соединение для аудиопотока
- Запускает процесс записи и анализа звука с микрофона
- Активирует мониторинг в реальном времени
- При детекции дрона отправляет через channels групповое уведомление
- Повторное нажатие кнопки останавливает мониторинг (прерывает соединие WebSocket)
...
```
### 2. Кнопка "Тест тревоги"   
**Назначение**: Тестовая тревога  
**Функционал**:
- Окрашивает список меток в красный цвет
- Метки на карте начинают мигать
- Отправляет данные на сервер
- Сервер имитирует тревогу

### 3. Кнопка "Стоп тревога"   
**Назначение**: Останавливает тревогу  
**Функционал**:
- Возвращает список меток в исходное состояние
